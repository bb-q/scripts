...
def SVGMatrixFromNode(node, context):
    ...
    m = m * Matrix.Translation(Vector((x, y, 0.0)))
    ...
    if len(context['rects']) > 1:
        if rect[0] != 0 and rect[1] != 0:
            m = m * Matrix.Scale(vw / rect[0], 4, Vector((1.0, 0.0, 0.0)))
            m = m * Matrix.Scale(vh / rect[1], 4, Vector((0.0, 1.0, 0.0)))
...
class SVGLoader(SVGGeometryContainer):
    ...
    def __init__(self, filepath, do_colormanage):
        ...
        m = Matrix()
        m = m * Matrix.Scale( 1.0/90.0 * 25.4/1000, 4, Vector((1.0, 0.0, 0.0)))
        m = m * Matrix.Scale(-1.0/90.0 * 25.4/1000, 4, Vector((0.0, 1.0, 0.0)))
